    ...
    return(
        <>
            ...
            {/* Show only on desktop */}
                ...
                {/* Show on all devices */}
                <div className="flex flex-row gap-1 items-center justify-start lg:justify-center">
                    <h1 className={`${MerriReg400.className} text-4xl text-gray-100`}>Irun</h1>
                    <GoDotFill className="text-amber-400"/>
                </div>

                {/* show only on mibile and tablets */}
                <div className="flex justify-end items-center">
                    <HiMenuAlt3 onClick={showMenu = ()=> setShowMenu(false)} className={`${showMenu ? 'flex' : 'hidden'} lg:hidden text-4xl text-gray-200`} />
                    <MdClose onClick={showMenu = ()=> setHideMenu(true)} className="hidden text-4xl text-gray-200"/>
                </div>
                
                {/* Show only on desktop */}
                    <div className="lg:flex hidden flex-row gap-6">
            ...
            {/* Monile-only nav */}
            <div className={`${!showMenu ? 'flex' : 'hidden'} flex-col gap-8 justify-center items-center lg:hidden min-h-1/2 w-full absolute top-[60px] left-0 z-10 bg-gray-800`}>
                <ul className="">
